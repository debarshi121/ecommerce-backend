// src/modules/inventory/constants/ReservationStatus.ts
//
// Mirrors chk_inventory_reservations_status in migration 006.
...
/**
 * Minutes a RESERVED reservation holds stock before it becomes eligible for
 * the expiry sweep (ReservationRepository.findPendingExpired).
 */
export const RESERVATION_DEFAULT_TTL_MINUTES = Number(
  process.env.INVENTORY_RESERVATION_TTL_MINUTES || 30,
);

/**
 * The reservation lifecycle as data. Every terminal state maps to an empty
 * list, so "no further transitions" is explicit rather than implied.
 *
 * `Record<ReservationStatusValue, ...>` makes the table exhaustive: a new
 * status without a row here is a compile error.
 */
export const RESERVATION_STATUS_TRANSITIONS: Record<
  ReservationStatusValue,
  readonly ReservationStatusValue[]
> = {
  PENDING: [ReservationStatus.RESERVED, ReservationStatus.FAILED],
  RESERVED: [
