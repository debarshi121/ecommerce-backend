// src/shared/repositories/InboxRepository.js

const crypto = require("crypto");

const InboxStatus = require("../constants/InboxStatus");
...
class InboxRepository {
  ...
  async findByEventId(eventId, queue, tx = null) {
    const query = `
      SELECT *
      FROM inbox_events
    ...
    const result = await executor.query(query, [eventId, queue]);
  ...
  async markProcessed(id, tx = null) {
    const query = `
      UPDATE inbox_events
    ...
    const result = await executor.query(query, [id, InboxStatus.PROCESSED]);
  ...
  async markFailed(event, tx = null) {
    const query = `
      INSERT INTO inbox_events (
        id,
        "eventId",
  ...
  async updateStatus(id, status, tx = null) {
    ...
    const result = await executor.query(query, [id, status]);

    return result.rows[0] || null;
  }

  async deleteOldProcessedEvents(days, tx = null) {
    const query = `
